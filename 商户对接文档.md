...
4. 最终签名值：`BA13C5CA2B914B8232F721A6ADEAE051`

**注意**：
- `body` 参数已移除，不再参与签名计算
- 空字符串和 null 值不参与签名计算
- 参数必须按 ASCII 码升序排序
- `api_secret` 不参与排序，直接拼接在最后作为 `key=api_secret`
...
**Content-Type**：`application/x-www-form-urlencoded`（POST请求时）
